...
// fk786
// rec03
// The following program implements the game of Minesweeper
// Users must select a row and column between 1 and 10 inclusive
// The number of rows and columns are set to 10
...
struct Tile {
	...
	int surroundingBombs; // If bomb is True, surroundingBombs is -1
};

class Minesweeper {
public:
	Minesweeper() {
		for (int x = 0; x < ROWS; ++x) { // Creates empty 10x10 board with empty tiles
		...
		for (int i = 0; i < BOMB_PROBABILITY; ++i) { // 10 of thoese empty tiles will be bombs, randomly selected
			int randX = rand() % ROWS;
			int randY = rand() % COLS;
			if (minesweeperVectorBoard[randX][randY].bomb != true) {
		...
		for (int x = 0; x < ROWS; ++x) {
			for (int y = 0; y < COLS; ++y) {
				if (minesweeperVectorBoard[x][y].bomb == true) { // Tile is a bomb
					...
					if (x < ROWS - 1) { // bottom  neighbor
						if (minesweeperVectorBoard[x + 1][y].bomb == true) {
					...
					if (y < COLS - 1) { // right neighbor
						if (minesweeperVectorBoard[x][y + 1].bomb == true) {
							numOfSurroundingBombs += 1;
						}
					}
					...
					if (x >= 1 && y < COLS - 1) { // top right
						if (minesweeperVectorBoard[x - 1][y + 1].bomb == true) {
							numOfSurroundingBombs += 1;
						}
					}
					if (x >= 1 && y >= 1) { // top left
					...
					minesweeperVectorBoard[x][y].surroundingBombs = numOfSurroundingBombs;
				}
			}
		}
	}
	...
	void checkSurroundingNeighbors(vector < vector < Tile> >& vectorTiles, int x, int y) {
		...
		tilesPlayed += 1;
		if (x < ROWS - 1) { // bottom  neighbor
			if (vectorTiles[x + 1][y].surroundingBombs == 0 && vectorTiles[x + 1][y].isVisible == false) {
				//vectorTiles[x + 1][y].isVisible = true;
				//tilesPlayed += 1;
				checkSurroundingNeighbors(vectorTiles, x + 1, y);
			}
			else if (vectorTiles[x + 1][y].surroundingBombs != 0 && vectorTiles[x + 1][y].isVisible == false) {
				vectorTiles[x + 1][y].isVisible = true;
				tilesPlayed += 1;
			}
		}
		...
		if (y < COLS - 1) { // right neighbor
			if (vectorTiles[x][y + 1].surroundingBombs == 0 && vectorTiles[x][y + 1].isVisible == false) {
				//vectorTiles[x][y+1].isVisible = true;
				//tilesPlayed +=1;
				checkSurroundingNeighbors(vectorTiles, x, y + 1);
			}
			else if (vectorTiles[x][y + 1].surroundingBombs != 0 && vectorTiles[x][y + 1].isVisible == false) {
				vectorTiles[x][y + 1].isVisible = true;
				tilesPlayed += 1;
			}
		}
		...
		if (x >= 1 && y < COLS - 1) { // top right
			if (vectorTiles[x - 1][y + 1].surroundingBombs == 0 && vectorTiles[x - 1][y + 1].isVisible == false) {
				//vectorTiles[x-1][y+1].isVisible = true;
				//tilesPlayed +=1;
				checkSurroundingNeighbors(vectorTiles, x - 1, y + 1);
			}
			else if (vectorTiles[x - 1][y + 1].surroundingBombs != 0 && vectorTiles[x - 1][y + 1].isVisible == false) {
				vectorTiles[x - 1][y + 1].isVisible = true;
				tilesPlayed += 1;
			}
		}
